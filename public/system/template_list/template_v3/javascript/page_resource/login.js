$(document).ready(function() {
    function e() {
        // gapi.auth2.getAuthInstance().signOut().then(function() {}), "undefined" != typeof FB && FB.logout(function(e) {});
        console.log('logout');
    ...
    $(".open_register").click(function() {
        ...
        var t = $("#email").val(),
            s = $("#password").val();
        "" == t || "" == s ? $("#login-status-wrapper").text("Please Enter Username And Password To Continue!!!").show() : ($("#login_auth_loading_image").show(), $(".data-utility-loader", $("#myModal_1")).show(), $("#login-status-wrapper").text("Please Wait!!!").hide(), $.post(app_base_url + "/auth/login", {
            email: t,
            password: s
        }, function(e) {
        ...
        e.preventDefault(), $("#recover-title-wrapper").hide(), $(".data-utility-loader", $("#myModal_2")).show(), $.post(app_base_url + "/auth/forgot_password", {
    ...
    $("#reset-password-trigger-book").on("click", function(e) {
        ...
        e.preventDefault(), $("#recover-title-wrapper-book").hide(), $(".data-utility-loader", $("#myModal_2")).show(), $.post(app_base_url + "/auth/forgot_password", {
    ...
    $("form#register_user_form #register_user_button").click(function(e) {
        ...
        }), 0 == $("#register_tc").prop("checked") ? (t = !1, $("#register_tc").addClass("invalid-ip")) : $("#register_tc").removeClass("invalid-ip"), 0 == t ? $(s).focus() : ($("#loading").removeClass("hide"), $.post(app_base_url + "/auth/register_on_light_box", $("form#register_user_form").serialize(), function(e) {
            var t = e.status,
                s = e.data;
            1 == t ? ($("#register-status-wrapper").empty().html(s), $("#register-status-wrapper").removeClass("hide"), $("#register_tc").prop("checked", !1), $("#loading").addClass("hide"), $('input[name="email"], select[name="country_code"], input[name="phone"], input[name="password"], input[name="confirm_password"]', "form#register_user_form").val("")) : ($("#register-error-msg").empty().html(s), $("#register-error-msg").removeClass("hide"), $("#loading").addClass("hide"))
        }))
